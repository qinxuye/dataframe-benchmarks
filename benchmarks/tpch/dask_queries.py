...
https://github.com/Bodo-inc/Bodo-examples/blob/master/examples/tpch/dask_queries.py
...
def load_lineitem(data_folder):
    data_path = data_folder + "/lineitem.pq/*.pq"
    df = pd.read_parquet(
        data_path,
    )
...
def load_part(data_folder):
    data_path = data_folder + "/part.pq/*.pq"
    df = pd.read_parquet(
        data_path,
    )
    return df
...
def load_orders(data_folder):
    data_path = data_folder + "/orders.pq/*.pq"
    df = pd.read_parquet(
        data_path,
    )
...
def load_customer(data_folder):
    data_path = data_folder + "/customer.pq/*.pq"
    df = pd.read_parquet(
        data_path,
    )
    return df
...
def load_partsupp(data_folder):
    data_path = data_folder + "/partsupp.pq/*.pq"
    df = pd.read_parquet(
        data_path,
    )
    return df
...
def run_queries(data_folder, client, n_workers):
    ...
    if len(client.scheduler_info()["workers"]) < n_workers:
        while (client.status == "running") and (
            ...
            time.sleep(5.0)
        print(
            "Cluster took %s seconds to fully spawn all workers"
            % (time.time() - t_start)
        )
...
def main():
    parser = argparse.ArgumentParser(description="Dask tpch-queries")
    parser.add_argument(
        "--folder",
        type=str,
